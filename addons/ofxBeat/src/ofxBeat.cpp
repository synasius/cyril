  ...
  if (fftInit) {
    ...
    if (bDetectBeat) {
      for (int i = 0; i < FFT_SUBBANDS; i++) {
        ...
        for (int b = 0; b < fft_size / FFT_SUBBANDS; b++) {
          fftVariance[i] +=
            pow(in_fft[i * (fft_size / FFT_SUBBANDS) + b] - fftSubbands[i], 2);
        }
...
float
ofxBeat::getMagnitude()
{
  return *magnitude;
}
float
ofxBeat::getBand(int i)
{
...
void
ofxBeat::updateBand(bool a, int b, int t)
{
  if (a) {
    beats[b] = 1;
...
int
ofxBeat::getBufferSize()
{
  return buffer_size;
}

void
ofxBeat::update(int _t)
{
  updateFFT();
...
void
ofxBeat::audioReceived(float* input, int bufferSize, int /* nChannels */)
{
  ...
  for (int i = 0; i < fft_size; i++) {
    float x = 0.085;
    magnitude_average[i] =
      (magnitude[i] * x) + (magnitude_average[i] * (1 - x));
  }
}

bool
ofxBeat::isBeat(int subband)
{
  return fftSubbands[subband] >
         averageEnergy[subband] * beatValueArray[subband];
}
...
bool
ofxBeat::isBeatRange(int low, int high, int threshold)
{
  int num = 0;
